  ...
  return (
    <Card
      className={cn(
        ...
        plan.recommended ? "border-[#ff5a74]/40 ring-1 ring-[#ff5a74]/20" : "border-white/60",
      )}
    >
      ...
      <CardContent className="space-y-4">
        <div className="flex items-end justify-between gap-3 rounded-2xl bg-slate-950 px-4 py-3 text-white">
          <div>
            <p className="text-[0.68rem] font-semibold uppercase tracking-[0.24em] text-white/60">Renews for</p>
...
export function SubscriptionPromptModal({ open, onClose }: SubscriptionPromptModalProps) {
  ...
  const startCheckout = async (planId: string) => {
    try {
      ...
      const data = (await response.json().catch(() => null)) as { error?: string; checkoutUrl?: string } | null;
  ...
  return (
    ...
    >
      <Card
        className="relative w-full max-w-6xl overflow-hidden border-white/10 bg-[linear-gradient(180deg,rgba(13,13,13,0.98),rgba(24,24,24,0.98))] p-6 text-white shadow-[0_32px_120px_rgba(0,0,0,0.6)]"
